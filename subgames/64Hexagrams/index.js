...
Page({
    ...
    initGame() {
        ...
        const gameHexagrams = shuffledHexagrams.slice(0, 3);
    ...
    checkMatch(selectedCards) {
        ...
        if (first.id === second.id) {
            ...
            // 为匹配的卦对分配颜色
            const colors = ['#4CAF50', '#2196F3', '#FF9800']; // 绿色、蓝色、橙色
            ...
            this.setData({
                ...
                progressText: `${newSuccessCount}/3`,
            ...
            // 显示匹配成功的卦象详情
            setTimeout(() => {
                const matchedHexagram = hexagramsData.find(h => h.id === first.id);
                if (matchedHexagram) {
                    this.showHexagramDetail(matchedHexagram);
                }
            }, 800);
    ...
    goToLearnHexagrams() {
        wx.navigateTo({
            url: '/subgames/64Hexagrams/pages/hexagram-list/hexagram-list'
        });
    },
